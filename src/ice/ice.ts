...
export class Connection {
  ...
  _tieBreaker = BigInt(new Uint64BE(randomBytes(64)).toString())
  state: IceState = 'new'
  dnsLookup?: DnsLookup
  ...
  private async getComponentCandidates(
    ...
    await Promise.all(
      addresses.map(async (address) => {
        ...
        try {
          await protocol.connectionMade(isIPv4(address), this.options.portRange, this.options.interfaceAddresses)
        } catch (error) {
    ...
    if (stunServer) {
      const stunPromises = this.protocols.map((protocol) =>
        new Promise<Candidate | void>(async (r, f) => {
          ...
          if (protocol.localCandidate?.host && isIPv4(protocol.localCandidate?.host)) {
    ...
    if (turnServer && turnUsername && turnPassword) {
      const turnCandidate = (async () => {
        const protocol = await createTurnEndpoint(turnServer, turnUsername, turnPassword, {
  ...
  private schedulingChecks() {
    // Ordinary Check
    {
      ...
      const pair = this.checkList
        .filter((pair) => {
          if (this.options.forceTurn && pair.protocol.type === 'stun') return false
          return true
        })
    ...
    new PCancelable(async (r, f, onCancel) => {
      ...
      try {
        while (!this.remoteIsLite && this.state !== 'closed') {
          ...
          await timers.setTimeout(CONSENT_INTERVAL * (0.8 + 0.4 * Math.random()) * 1000, undefined, {
  ...
  requestReceived(message: Message, addr: Address, protocol: Protocol, rawData: Buffer) {
    ...
    } else if (!iceControlling && message.attributesKeys.includes('ICE-CONTROLLED')) {
    ...
    if (this.options.filterStunResponse && !this.options.filterStunResponse(message, addr, protocol)) {
      return
    }

    // # send binding response
    const response = new Message(methods.BINDING, classes.RESPONSE, message.transactionId)
    response
  ...
  private findPair(protocol: Protocol, remoteCandidate: Candidate) {
    const pair = this.checkList.find(
      (pair) => isEqual(pair.protocol, protocol) && isEqual(pair.remoteCandidate, remoteCandidate),
    )
    return pair
  }
  ...
  private checkComplete(pair: CandidatePair) {
    ...
    if (pair.state === CandidatePairState.SUCCEEDED) {
      ...
      for (const p of this.checkList) {
        if (p.localCandidate.foundation === pair.localCandidate.foundation && p.state === CandidatePairState.FROZEN) {
  ...
  private buildRequest(pair: CandidatePair, nominate: boolean) {
    ...
    request.setAttribute('USERNAME', txUsername).setAttribute('PRIORITY', candidatePriority(pair.component, 'prflx'))
    if (this.iceControlling) {
  ...
  private respondError(request: Message, addr: Address, protocol: Protocol, errorCode: [number, string]) {
    const response = new Message(request.messageMethod, classes.ERROR, request.transactionId)
    response
...
export function sortCandidatePairs(pairs: CandidatePair[], iceControlling: boolean) {
  pairs.sort(
    (a, b) =>
...
export function candidatePairPriority(local: Candidate, remote: Candidate, iceControlling: boolean) {
...
function isAutoconfigurationAddress(info: os.NetworkInterfaceInfo) {
  return normalizeFamilyNodeV18(info.family) === 4 && info.address?.startsWith('169.254.')
}
...
export async function serverReflexiveCandidate(protocol: Protocol, stunServer: Address) {
  // """
