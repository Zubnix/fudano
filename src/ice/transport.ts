...
import { findPort, interfaceAddress, InterfaceAddresses } from '../common/network'
...
export class UdpTransport implements Transport {
  ...
  static async init(type: SocketType, portRange?: [number, number], interfaceAddresses?: InterfaceAddresses) {
  ...
  private async init() {
    ...
    if (this.portRange) {
      const port = await findPort(this.portRange[0], this.portRange[1], this.type, this.interfaceAddresses)
