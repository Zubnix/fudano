...
function packAddress(value: Address) {
  ...
  return Buffer.concat([Buffer.from(jspack.Pack('!BBH', [0, protocol, value[1]])), nodeIp.toBuffer(address)])
}
...
function xorAddress(data: Buffer, transactionId: Buffer) {
  const xPad = [...jspack.Pack('!HI', [COOKIE >> 16, COOKIE]), ...transactionId]
...
export function packErrorCode(value: [number, string]) {
  const pack = Buffer.from(jspack.Pack('!HBB', [0, Math.floor(value[0] / 100), value[0] % 100]))
...
const packUnsignedShort = (value: number) =>
  Buffer.concat([Buffer.from(jspack.Pack('!H', [value])), Buffer.from('\x00\x00')])
const unpackUnsignedShort = (data: Buffer) => jspack.Unpack('!H', data.slice(0, 2))[0]
...
export type ATTRIBUTE = [number, AttributeKey, (...args: any) => Buffer, (...args: any) => any]
