...
import { classes, COOKIE, FINGERPRINT_LENGTH, FINGERPRINT_XOR, HEADER_LENGTH, INTEGRITY_LENGTH, methods } from './const'

export function parseMessage(data: Buffer, integrityKey?: Buffer): Message | undefined {
  if (data.length < HEADER_LENGTH) {
    return undefined
  }
  const [messageType, length] = jspack.Unpack('!HHI', data.slice(0, HEADER_LENGTH))

  const transactionId = Buffer.from(data.slice(HEADER_LENGTH - 12, HEADER_LENGTH))
  ...
  for (let pos = HEADER_LENGTH; pos <= data.length - 4; ) {
    ...
    if (attributesTypes.includes(attrType.toString())) {
      ...
      if (attrUnpack.name === unpackXorAddress.name) {
        attributeRepository.setAttribute(attrName as AttributeKey, attrUnpack(payload, transactionId))
      } else {
        attributeRepository.setAttribute(attrName as AttributeKey, attrUnpack(payload))
      }
...
export class Message extends AttributeRepository {
  ...
  get bytes() {
    ...
    const buf = Buffer.from(jspack.Pack('!HHI', [this.messageMethod | this.messageClass, data.length, COOKIE]))
  ...
  messageIntegrity(key: Buffer) {
    const checkData = setBodyLength(this.bytes, this.bytes.length - HEADER_LENGTH + INTEGRITY_LENGTH)
    return Buffer.from(createHmac('sha1', key).update(checkData).digest('hex'), 'hex')
  }
...
const setBodyLength = (data: Buffer, length: number) => {
  return Buffer.concat([data.slice(0, 2), Buffer.from(jspack.Pack('!H', [length])), data.slice(4)])
}

function messageFingerprint(data: Buffer) {
  const checkData = setBodyLength(data, data.length - HEADER_LENGTH + FINGERPRINT_LENGTH)
...
function messageIntegrity(data: Buffer, key: Buffer) {
  const checkData = setBodyLength(data, data.length - HEADER_LENGTH + INTEGRITY_LENGTH)
  return Buffer.from(createHmac('sha1', key).update(checkData).digest('hex'), 'hex')
}
