...
export class StunProtocol implements Protocol {
  ...
  connectionMade = async (useIpv4: boolean, portRange?: [number, number], interfaceAddresses?: InterfaceAddresses) => {
    if (useIpv4) {
      this.transport = await UdpTransport.init('udp4', portRange, interfaceAddresses)
    } else {
      this.transport = await UdpTransport.init('udp6', portRange, interfaceAddresses)
    }
  ...
  async request(request: Message, addr: Address, integrityKey?: Buffer, retransmissions?: number) {
    // """
    ...
    const transaction: Transaction = new Transaction(request, addr, this, retransmissions)
