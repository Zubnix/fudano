...
class TurnTransport implements Protocol {
  ...
  private datagramReceived = (data: Buffer, addr: Address) => {
    ...
    if (
      (message?.messageClass === classes.RESPONSE || message?.messageClass === classes.ERROR) &&
  ...
  async request(request: Message, addr: Address, integrityKey?: Buffer): Promise<[Message, Address]> {
...
class TurnClient implements Protocol {
  ...
  private handleSTUNMessage(data: Buffer, addr: Address) {
    try {
      ...
      if (message.messageClass === classes.RESPONSE || message.messageClass === classes.ERROR) {
  ...
  async connect() {
    ...
    withoutCred.setAttribute('LIFETIME', this.lifetime).setAttribute('REQUESTED-TRANSPORT', UDP_TRANSPORT)

    const err: TransactionFailed = await this.request(withoutCred, this.server).catch((e) => e)
...
export async function createTurnEndpoint(
  ...
  const transport = await UdpTransport.init('udp4', portRange, interfaceAddresses)
...
export function makeIntegrityKey(username: string, realm: string, password: string) {
