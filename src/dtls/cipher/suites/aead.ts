import { createCipheriv, createDecipheriv, CipherCCMTypes } from 'node:crypto'
...
import binary_data from 'binary-data'
...
const err = debug('werift-dtls : packages/dtls/src/cipher/suites/aead.ts : err')
...
export default class AEADCipher extends Cipher {
  ...
  encrypt(type: SessionTypes, data: Buffer, header: CipherHeader) {
    ...
    const cipher = createCipheriv(this.blockAlgorithm as CipherCCMTypes, writeKey, iv, {
  ...
  decrypt(type: SessionTypes, data: Buffer, header: CipherHeader) {
    ...
    const decipher = createDecipheriv(this.blockAlgorithm as CipherCCMTypes, writeKey, iv, {
      authTagLength: this.authTagLength,
    })
    ...
    try {
      ...
      err('decrypt failed', error, type, dumpBuffer(data), header, this.summary)
      throw error
    }
  }
}
