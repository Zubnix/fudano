...
export function prfPreMasterSecret(publicKey: Buffer, privateKey: Buffer, curve: NamedCurveAlgorithms) {
  switch (curve) {
...
export function prfPHash(secret: Buffer, seed: Buffer, requestedLegth: number, algorithm = 'sha256') {
...
export function prfMasterSecret(preMasterSecret: Buffer, clientRandom: Buffer, serverRandom: Buffer) {
  const seed = Buffer.concat([Buffer.from('master secret'), clientRandom, serverRandom])
  return prfPHash(preMasterSecret, seed, 48)
}

export function prfExtendedMasterSecret(preMasterSecret: Buffer, handshakes: Buffer) {
...
export function prfVerifyData(masterSecret: Buffer, handshakes: Buffer, label: string, size = 12) {
