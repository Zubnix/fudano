...
import { HashAlgorithm, NamedCurveAlgorithmList, SignatureAlgorithm, SignatureHash } from './cipher/const'
...
export class DtlsSocket {
          ...
          case ContentType.handshake:
            {
              ...
              const assembled = Object.values(
                handshakes.reduce((acc: { [type: string]: FragmentedHandshake[] }, cur) => {
  ...
  private setupExtensions() {
    {
      ...
      if (this.options.srtpProfiles && this.options.srtpProfiles.length > 0) {
        const useSrtp = UseSRTP.create(this.options.srtpProfiles, Buffer.from([0x00]))
  ...
  extractSessionKeys(keyLength: number, saltLength: number) {
    ...
    const { clientKey, serverKey, clientSalt, serverSalt } = decode(keyingMaterial, {
