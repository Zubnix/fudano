...
import { CurveType, HashAlgorithm, SignatureAlgorithm } from '../../cipher/const'
...
export class Flight4 extends Flight {
  ...
  async exec(clientHello: FragmentedHandshake, certificateRequest = false) {
  ...
  private sendServerHello() {
    ...
    if (this.srtp.srtpProfile) {
      extensions.push(UseSRTP.create([this.srtp.srtpProfile], Buffer.from([0x00])).extension)
    }
