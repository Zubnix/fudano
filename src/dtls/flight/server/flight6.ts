...
export class Flight6 extends Flight {
  constructor(udp: TransportContext, dtls: DtlsContext, private cipher: CipherContext) {
    super(udp, dtls, 6)
  }
...
const handlers: {
  [key: number]: (contexts: { dtls: DtlsContext; cipher: CipherContext }) => (message: any) => void
} = {}
  ...
  (message: ClientKeyExchange) => {
    ...
    cipher.masterSecret =
      dtls.options.extendedMasterSecret && dtls.remoteExtendedMasterSecret
        ...
        : prfMasterSecret(preMasterSecret, cipher.remoteRandom.serialize(), cipher.localRandom.serialize())
