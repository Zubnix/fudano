...
import { prfExtendedMasterSecret, prfMasterSecret, prfPreMasterSecret } from '../../cipher/prf'
...
const log = debug('werift-dtls : packages/dtls/src/flight/client/flight5.ts : log')

export class Flight5 extends Flight {
  constructor(udp: TransportContext, dtls: DtlsContext, private cipher: CipherContext, private srtp: SrtpContext) {
    super(udp, dtls, 5, 7)
  }
  ...
  private sendClientKeyExchange() {
    ...
    this.cipher.masterSecret =
      this.dtls.options.extendedMasterSecret && this.dtls.remoteExtendedMasterSecret
        ? prfExtendedMasterSecret(preMasterSecret, handshakes)
        : prfMasterSecret(preMasterSecret, this.cipher.localRandom.serialize(), this.cipher.remoteRandom.serialize())
...
const handlers: {
  [key: number]: (contexts: { dtls: DtlsContext; cipher: CipherContext; srtp: SrtpContext }) => (message: any) => void
} = {}
