...
export class FragmentedHandshake {
  ...
  chunk(maxFragmentLength?: number): FragmentedHandshake[] {
    ...
    if (totalLength === 0)
      return [new FragmentedHandshake(this.msg_type, totalLength, this.message_seq, start, 0, this.fragment)]
  ...
  static findAllFragments(fragments: FragmentedHandshake[], type: HandshakeType): FragmentedHandshake[] {
    ...
    return fragments.filter((f) => {
      return (
        f.msg_type === reference.msg_type && f.message_seq === reference.message_seq && f.length === reference.length
      )
    })
  }
}
