...
export class CipherContext {
  ...
  encryptPacket(pkt: DtlsPlaintext) {
    ...
    const enc = this.cipher.encrypt(this.sessionType, pkt.fragment, {
      ...
      version: decode(Buffer.from(encode(header.protocolVersion, ProtocolVersion).slice()), { version: types.uint16be })
        .version,
      epoch: header.epoch,
  ...
  private valueKeySignature(clientRandom: Buffer, serverRandom: Buffer, publicKey: Buffer, namedCurve: number) {
