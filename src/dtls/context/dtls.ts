...
export class DtlsContext {
  ...
  checkHandshakesExist = (handshakes: number[]) =>
    !handshakes.find((type) => this.sortedHandshakeCache.find((h) => h.msg_type === type) == undefined)

  bufferHandshakeCache(handshakes: FragmentedHandshake[], isLocal: boolean, flight: number) {
