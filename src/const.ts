import { HashAlgorithms, SignatureAlgorithm } from './dtls/cipher/const'
import { ProtectionProfileAeadAes128Gcm, ProtectionProfileAes128CmHmacSha1_80 } from './dtls/context/srtp'
...
export const DTLS_SETUP_ROLE: { [key: string]: DtlsRole } = Object.keys(DTLS_ROLE_SETUP).reduce((acc, cur) => {
  const key = (DTLS_ROLE_SETUP as any)[cur]
  acc[key] = cur
  return acc
}, {} as any)
export const FMTP_INT_PARAMETERS = ['apt', 'max-fr', 'max-fs', 'maxplaybackrate', 'minptime', 'stereo', 'useinbandfec']
