...
import crc32c from 'turbo-crc32/crc32c'
...
export class Chunk {
  ...
  constructor(public flags = 0, private _body: Buffer | undefined = Buffer.from('')) {}
  ...
  get bytes() {
    ...
    const data = Buffer.concat([
      Buffer.from(jspack.Pack('!BBH', [this.type, this.flags, this.body.length + 4])),
      this.body,
...
export class BaseInitChunk extends Chunk {
  ...
  constructor(public flags = 0, body?: Buffer) {
    ...
    if (body) {
      ;[this.initiateTag, this.advertisedRwnd, this.outboundStreams, this.inboundStreams, this.initialTsn] =
...
export class ForwardTsnChunk extends Chunk {
  ...
  constructor(public flags = 0, body: Buffer | undefined) {
    ...
    if (body) {
      ...
      while (pos < body.length) {
        this.streams.push(jspack.Unpack('!HH', body.slice(pos)) as [number, number])
        pos += 4
      }
    } else {
  ...
  get body() {
    ...
    return Buffer.concat([body, ...this.streams.map(([id, seq]) => Buffer.from(jspack.Pack('!HH', [id, seq])))])
  }
}
...
export class DataChunk extends Chunk {
  ...
  constructor(public flags = 0, body: Buffer | undefined) {
    super(flags, body)
    if (body) {
      ;[this.tsn, this.streamId, this.streamSeqNum, this.protocol] = jspack.Unpack('!LHHL', body)
  ...
  get bytes() {
    ...
    if (length % 4) {
      data = Buffer.concat([data, ...[...Array(padL(length))].map(() => Buffer.from('\x00'))])
    }
    return data
  }
}
...
export class SackChunk extends Chunk {
  ...
  constructor(public flags = 0, body: Buffer | undefined) {
    ...
    if (body) {
      const [cumulativeTsn, advertisedRwnd, nbGaps, nbDuplicates] = jspack.Unpack('!LLHH', body)
      ...
      ;[...Array(nbGaps)].forEach(() => {
        this.gaps.push(jspack.Unpack('!HH', body.slice(pos)) as [number, number])
        pos += 4
      })
  ...
  get bytes() {
    ...
    data = Buffer.concat([data, ...this.gaps.map((gap) => Buffer.from(jspack.Pack('!HH', gap)))])
    data = Buffer.concat([data, ...this.duplicates.map((tsn) => Buffer.from(jspack.Pack('!L', [tsn])))])
    return data
  }
}
...
export const CHUNK_BY_TYPE = CHUNK_CLASSES.reduce((acc: { [key: string]: typeof Chunk }, cur) => {
  acc[cur.type] = cur
  return acc
}, {})
...
function encodeParams(params: [number, Buffer][]) {
  ...
  params.forEach(([type, value]) => {
    ...
    body = Buffer.concat([body, padding, Buffer.from(jspack.Pack('!HH', [type, length])), value])
...
export function serializePacket(sourcePort: number, destinationPort: number, verificationTag: number, chunk: Chunk) {
  const header = Buffer.from(jspack.Pack('!HHL', [sourcePort, destinationPort, verificationTag]))
  const body = chunk.bytes

  const checksum: number = crc32c(Buffer.concat([header, Buffer.from('\x00\x00\x00\x00'), body]))
