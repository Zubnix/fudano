...
export class OutgoingSSNResetRequestParam {
  ...
  get bytes() {
    const data = Buffer.from(jspack.Pack('!LLL', [this.requestSequence, this.responseSequence, this.lastTsn]))

    return Buffer.concat([data, ...this.streams.map((stream) => Buffer.from(jspack.Pack('!H', [stream])))])
  }

  static parse(data: Buffer) {
    const [requestSequence, responseSequence, lastTsn] = jspack.Unpack('!LLL', data)
    const stream = range(12, data.length, 2).map((pos) => jspack.Unpack('!H', data.slice(pos))[0])
...
export class StreamAddOutgoingParam {
  ...
  get bytes() {
    return Buffer.from(jspack.Pack('!LHH', [this.requestSequence, this.newStreams, 0]))
  }
...
export class ReconfigResponseParam {
  ...
  get bytes() {
    return Buffer.from(jspack.Pack('!LL', [this.responseSequence, this.result]))
  }
