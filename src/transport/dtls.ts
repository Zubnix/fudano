...
import { DtlsClient } from '../dtls/client'
import { DtlsSocket } from '../dtls/socket'
import { DtlsServer } from '../dtls/server'
import { Transport } from '../dtls/transport'
import { HashAlgorithm, NamedCurveAlgorithm, SignatureAlgorithm, SignatureHash } from '../dtls/cipher/const'
import { CipherContext } from '../dtls/context/cipher'
import { Profile } from '../dtls/context/srtp'
import { Connection } from '../ice/ice'
// import {
//   isRtcp,
//   RtcpPacket,
//   RtcpPacketConverter,
...
export const DtlsStates = ['new', 'connecting', 'connected', 'closed', 'failed'] as const
...
export class RTCCertificate {
  ...
  constructor(privateKeyPem: string, public certPem: string, public signatureHash: SignatureHash) {
  ...
  getFingerprints(): RTCDtlsFingerprint[] {
    return [
      new RTCDtlsFingerprint('sha-256', fingerprint(Certificate.fromPEM(Buffer.from(this.certPem)).raw, 'sha256')),
    ]
  }
}
...
export class RTCDtlsParameters {
  constructor(public fingerprints: RTCDtlsFingerprint[] = [], public role: 'auto' | 'client' | 'server') {}
}
